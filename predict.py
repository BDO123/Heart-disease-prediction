...
looping=input("Test for a person :Yes    -->")
loop=looping.upper()
print(loop)
output={'age':[],'sex':[],'fbs':[],'thalach':[],'cp':[],'exang':[],'predicted':[]}
while(loop=="YES"):
    m=[]
    for i in range(6):
        print("-->",dataset_select.columns[i].upper(), end=" ")
        m.append(int(input()))
        if(m[i]<=1):
            m[i]=bool(m[i])
        output[dataset_select.columns[i]].append(m[i])
    m=np.array(m)
    inp=m.reshape(1,-1)
    ...
    #print(test_healthy)
    a=list(test_healthy)
    output['predicted'].append(a[0])
    print("Predicted value  : ",*a)
    print("")
    looping=input("Again  test for a person :Yes  ,else: NO  --> ")
    loop=looping.upper()
#print(a[0])
